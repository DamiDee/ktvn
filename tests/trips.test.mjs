...
test("midnight rolls back a UTC day but stays the same calendar day in Abuja", () => {
  ...
  assert.equal(tripCanBook({ ...trip, departure_time: timestamp }, route, saturday), true);
});
...
test("travel is offered on any day of the week, not Sundays alone", () => {
  assert.equal(tripCanBook(trip, route, saturday), true);
  // Monday, Wednesday, Friday — every upcoming departure is bookable.
  for (const day of ["2030-01-07", "2030-01-09", "2030-01-11"]) {
    assert.equal(tripCanBook({ ...trip, departure_time: `${day}T09:00:00+01:00` }, route, saturday), true);
  }
});
test("the calendar day is read in Abuja time, not UTC or the device timezone", () => {
...
test("booking requires the matching, free, upcoming, not-started trip", () => {
  for (const status of ["InProgress", "Completed", "Cancelled"]) assert.equal(tripCanBook({ ...trip, status }, route, saturday), false);
  assert.equal(tripCanBook(trip, { ...route, id: "different" }, saturday), false);
  assert.equal(tripCanBook(trip, { ...route, fare: 100 }, saturday), false);
  assert.equal(tripCanBook(trip, route, Date.parse(trip.departure_time)), false);
  assert.equal(tripCanBook({ ...trip, departure_time: "invalid" }, route, saturday), false);
});
test("one reusable route gives each departure its own journey and ticket identity", () => {
