...
export function OperationsForm({ panel, points, routes, trips, buses, busy, error, onSubmit }: {
  panel: "route" | "point" | "bus" | "assign"; points: ApiPoint[]; routes: ApiRoute[]; trips: ApiTrip[]; buses: ApiBus[]; busy: boolean; error: string;
  ...
  return <form className="space-y-4" onSubmit={async (event) => {
    ...
    {panel === "point" ? <>
      <Input label="Location name" name="name" required value={name} onChange={(e) => setName(e.target.value)} /><Input label="Meeting landmark" name="landmark" required /><Input label="Directions" name="description" required />
      <MapPicker value={location} onChange={setLocation} onPlaceSelect={(place) => { if (!name) setName(place.name); }} />
      ...
      <Select label="Scheduled trip" name="trip" required options={[{ value: "", label: "Choose a trip" }, ...trips.filter((t) => t.status === "NotStarted").map((t) => ({ value: t.id, label: `${routes.find((r) => r.id === t.route_id)?.name ?? t.route_id} · ${watParts(t.departure_time).date} ${watParts(t.departure_time).time}` }))]} />
    </> : <>
      <p className="text-sm text-ink-secondary">Create this route once. Schedule future Sunday trips from the Trips module without recreating the route.</p>
      <Input label="Route name" name="name" placeholder="Lugbe to Koinonia Sunday Service" required /><Input label="Boarding instructions" name="description" required />
      <Select label="Boarding location" value={start} onChange={(e) => setStart(e.target.value)} required options={pointOptions} /><Select label="Destination" value={end} onChange={(e) => setEnd(e.target.value)} required options={pointOptions} />
