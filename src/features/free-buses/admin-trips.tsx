...
import { apiTimestamp, SUNDAY_TRAVEL_ONLY, watInputToApiTime, watParts } from "@/lib/trips";
import type { ApiTrip, ApiRoute, ApiBus } from "@/types/freebus-api";

export function AdminTrips({ initialRoute = "" }: { initialRoute?: string }) {
  ...
  return <>
    ...
    <div className="mb-5 flex flex-wrap items-center gap-3 rounded-2xl border border-gold-500/25 bg-gold-500/5 px-5 py-4 text-sm text-ink-secondary"><StatusChip tone="pending">{SUNDAY_TRAVEL_ONLY ? "Sunday travel" : "All travel days"}</StatusChip><span>Members may book any day for a published {SUNDAY_TRAVEL_ONLY ? "Sunday " : ""}departure. All times are Abuja (WAT).</span></div>
    ...
    <Modal open={Boolean(editor)} onClose={() => { if (!busy) setEditor(null); }} title={editor === "new" ? `Schedule ${SUNDAY_TRAVEL_ONLY ? "a Sunday" : "a"} trip` : "Edit trip"} size="lg">
      {editor ? <TripForm key={editor === "new" ? "new" : editor.id} trip={editor === "new" ? undefined : editor} initialRoute={initialRoute} routes={routes.data} buses={buses.data} busy={busy} error={error} save={(body) => run(() => freebusRequest(editor === "new" ? "trips" : `trips/${editor.id}`, { method: editor === "new" ? "POST" : "PATCH", body: JSON.stringify(body) }))} /> : null}
    </Modal>
...
function TripForm({ trip, routes, buses, initialRoute, busy, error, save }: { trip?: ApiTrip; routes: ApiRoute[]; buses: ApiBus[]; initialRoute: string; busy: boolean; error: string; save: (body: Record<string, unknown>) => Promise<void> }) {
  ...
  return <form className="space-y-4" onSubmit={async (event) => {
    ...
    if (SUNDAY_TRAVEL_ONLY && !watParts(departureTime).sunday) { setValidation("Choose a Sunday departure in Abuja (WAT). Members can book on any day."); return; }
    ...
    <Input label="Departure (WAT)" name="departure" type="datetime-local" required defaultValue={departure ? `${departure.date}T${departure.time.slice(0, 5)}` : undefined} />
    <Input label="Expected arrival (WAT)" name="arrival" type="datetime-local" required defaultValue={arrival ? `${arrival.date}T${arrival.time.slice(0, 5)}` : undefined} />
